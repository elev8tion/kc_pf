...
export default function Model({ imageUrl = '/images/profile.png' }) {
  ...
  const { scene: imageScene, camera: imageCamera } = Images(viewport, imageUrl);
  ...
  function Images(viewport: any, imageUrl: string) {
    const scene = new THREE.Scene();
    ...
    const texture1 = useTexture(imageUrl);
    const material1 = new THREE.MeshBasicMaterial({ map: texture1 });
    const image1 = new THREE.Mesh(geometry, material1);

    // Position and scale the single image (adjust these values as needed)
    image1.position.x = 0;
    image1.position.y = 0;
    image1.position.z = 1;
    // Maintain 3:4 aspect ratio for the profile image
    const imageWidth = viewport.width / 2.5;
    const imageHeight = imageWidth * (4/3);
    image1.scale.x = imageWidth;
    image1.scale.y = imageHeight;
